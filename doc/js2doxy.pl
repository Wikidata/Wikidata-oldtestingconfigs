...
sub parse_function
{
    ...
    if( $last_doc )
    {
        ...
        {
            if( exists $fnContext->{constructor} )
            {
            ...
            {
                $fnContext->{constructor} = $last_doc;
    ...
    for( $context->{otype} )
    {
        /^$OT_FUNCTION$/ && do
        {
            $context->{otype} = $OT_CLASS
                if(   ( exists $context->{constructor} )
                   || (   ( exists $context->{doc} )
                       && ( exists $context->{doc}{constructor} )));
        }
    }
}
...
sub generate_function
{
    ...
    $constructor = $func->{constructor}
        if(   exists $func->{constructor}
           && (   $func->{otype} == $OT_CLASS
               || $func->{otype} == $OT_INTERFACE ));
    $constructor = $func->{doc}{constructor}
        if(   exists $func->{doc}
           && exists $func->{doc}{constructor}
           && (   $func->{otype} == $OT_CLASS
               || $func->{otype} == $OT_INTERFACE ));
    if( $doc )
    {
    ...
    if( $constructor )
    {
        syntax_err( "Documentation for $npref$name is not for a constructor." )
            if(   $constructor->{otype} != $OT_CONSTRUCTOR
               && $constructor->{otype} != $OT_UNKNOWN );
        print( "\n".$constructor->{text} );
        $rtype = "void";
        $argtypes = $constructor->{args} if( exists $constructor->{args} );
    }
